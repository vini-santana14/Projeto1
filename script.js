...
const tabRecords = [
    {
        company: {
            src: "recursos/emp/aws.png",
            name: "Amazon",
        },
        role: 'Senior DevOps Enginner',
        type: 'on-site',
        salary: '$220k - $300k / year',
        location: 'New York, United States',
        applicants: ['lilia', 'drew', 'melissa'],
...
const filter = {
    ["all"]: () => true,
    ["on-site"]: (record) => record.type.include("on-site"),
    ["remote"]: (record) => record.type.include("remote"),
    ["hybrid"]: (record) => record.type.include("hybrid"),
}
...
const generateTabItems = (elem, tabContent) => {
    ...
    const mappedRecords = tabRecords.filter(filterFunction).map(({ company, ...record }) => {
        return DOMPurify.sanitize(
              ...
              <div class="job__main">
                ...
                <div class"job__description">
                  <div class="job__name">
                   ...
                   <div class="job__description">
                     <div class="job__name">
                      <diiv class="job__role">
                       ${record.role} (${record.type})
                       </div>
                       <button type="button" class="job__bookmark">
                         <svg xmlns="http://w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="#333531"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-bookmark"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M18 7v14l-6 -4l-6 4v-14a4 4 0 0 1 4 -4h4a4 4 0 0 1 4 4z" /></svg>
                      </button>
                    </div>
                    <div class="job__location">
                     <img src="recursos/location.svg" alt="location">
                     ${record.location}
                    </div>
                </div>
            </div>
            <div class="job__applicants">
              ${record.applicants.map((applicant) => {
                return (
                    `<img src="recursos/users/${applicant}.jpg" alt="${applicant.name}" class="job__applicant">`
                )
              }).join("")}
              +${record.applicantionsCount} applicants
              </div>
              <div class="job__salary">
              ${record.salary}
              </div>
    ...
    tab.Content.innerHTML = mappedRecords.join("")
}

//? handle proper slection for initial load
const currentHash = window.location.hash
...
const activeTab = document.querySelector(`#${activeLink.id}`)
